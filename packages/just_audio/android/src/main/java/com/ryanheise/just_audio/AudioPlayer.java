...
public class AudioPlayer implements MethodCallHandler, Player.Listener, MetadataOutput {
    ...
    private final EventChannel visualizerEventChannel;
    private EventChannel.EventSink visualizerEventSink;
    private TeeAudioProcessor visualizerProcessor;
    private int visualizerChannelCount = 2;
    private final Handler mainHandler = new Handler(Looper.getMainLooper());
    private final ZeroAllocRadix2FFT fftEngine = new ZeroAllocRadix2FFT(512);
    private final float[] pcmRingBuffer = new float[32768];
    private int pcmWriteIndex = 0;
    private int pcmReadIndex = 0;
    private final Object pcmLock = new Object();
    private final float[] snapshotBuffer = new float[512];
    private boolean isVisualizerLoopRunning = false;
    private static final int HOP_SIZE = 706; // 44100 * 0.016s = ~706 samples per 16ms frame
    private static final int SYNC_LATENCY_SAMPLES = 6615; // ~150ms hardware AudioTrack latency sync

    private final Runnable visualizerRunnable = new Runnable() {
        @Override
        public void run() {
            if (visualizerEventSink == null || !isVisualizerLoopRunning) {
                return;
            }
            if (player != null && player.getPlayWhenReady() && player.getPlaybackState() == Player.STATE_READY) {
                synchronized (pcmLock) {
                    // Continuous STFT hop: smoothly advance by ~706 samples every frame
                    pcmReadIndex = (pcmReadIndex + HOP_SIZE) % pcmRingBuffer.length;

                    // Sync check: ensure read head matches hardware AudioTrack output timing (~150ms)
                    int distance = (pcmWriteIndex - pcmReadIndex + pcmRingBuffer.length) % pcmRingBuffer.length;
                    if (distance < 2000 || distance > 18000) {
                        pcmReadIndex = (pcmWriteIndex - SYNC_LATENCY_SAMPLES + pcmRingBuffer.length) % pcmRingBuffer.length;
                    }

                    for (int i = 0; i < 512; i++) {
                        snapshotBuffer[i] = pcmRingBuffer[(pcmReadIndex + i) % pcmRingBuffer.length];
                    }
                }
                List<Double> bands = fftEngine.process(snapshotBuffer);
                if (bands != null && visualizerEventSink != null) {
                    visualizerEventSink.success(bands);
                }
            }
    ...
    public AudioPlayer(
        ...
        visualizerEventChannel = new EventChannel(messenger, "com.ryanheise.just_audio.visualizer/" + id);
        visualizerEventChannel.setStreamHandler(new EventChannel.StreamHandler() {
            @Override
    ...
    private void initVisualizerProcessor() {
        if (visualizerProcessor != null) {
            return;
        }
        visualizerProcessor = new TeeAudioProcessor(new TeeAudioProcessor.AudioBufferSink() {
            @Override
            public void flush(int sampleRateHz, int channelCount, int encoding) {
                visualizerChannelCount = channelCount > 0 ? channelCount : 2;
            }
            ...
            public void handleBuffer(ByteBuffer buffer) {
                ...
                try {
                    ByteBuffer slice = buffer.asReadOnlyBuffer().order(ByteOrder.LITTLE_ENDIAN);
                    int frameBytes = Math.max(1, visualizerChannelCount) * 2;
                    int available = slice.remaining() / frameBytes;
                    if (available > 0) {
                        synchronized (pcmLock) {
                            for (int i = 0; i < available; i++) {
                                short left = slice.getShort();
                                short right = visualizerChannelCount > 1 ? slice.getShort() : left;
                                for (int c = 2; c < visualizerChannelCount; c++) {
                                    slice.getShort();
                                }
                                pcmRingBuffer[pcmWriteIndex] = (((float) left + (float) right) / 2.0f) / 32768.0f;
                                pcmWriteIndex = (pcmWriteIndex + 1) % pcmRingBuffer.length;
                            }
                        }
                    }
                } catch (Exception e) {
                    // Ignore transient read errors during stream transitions
                }
            }
        });
    }
    ...
    public void dispose() {
        ...
        stopVisualizerLoop();
        visualizerEventChannel.setStreamHandler(null);
        visualizerEventSink = null;
    ...
    static class ZeroAllocRadix2FFT {
        private final int n;
        private final int numBands = 32;
        private final float[] real;
        private final float[] imag;
        private final float[] cosTable;
        private final float[] sinTable;
        private final float[] windowTable;
        private final int[] bitReversal;
        private final int[] bandStartIndices;
        private final int[] bandEndIndices;

        public ZeroAllocRadix2FFT(int n) {
            this.n = n;
            this.real = new float[n];
            this.imag = new float[n];
            this.cosTable = new float[n / 2];
            this.sinTable = new float[n / 2];
            this.windowTable = new float[n];
            this.bitReversal = new int[n];
            ...
            for (int i = 0; i < n; i++) {
                windowTable[i] = (float) (0.5 * (1.0 - Math.cos(2.0 * Math.PI * i / (n - 1))));
            }

            int bits = 31 - Integer.numberOfLeadingZeros(n);
            for (int i = 0; i < n; i++) {
                bitReversal[i] = reverseBits(i, bits);
            }

            bandStartIndices = new int[numBands];
            bandEndIndices = new int[numBands];
            int minBin = 1;
            int maxBin = (int) Math.round(16000.0 / (44100.0 / n)); // ~186 (~16 kHz upper cutoff)
            for (int i = 0; i < numBands; i++) {
                double p0 = (double) i / numBands;
                double p1 = (double) (i + 1) / numBands;
                int start = (int) Math.round(minBin * Math.pow((double) maxBin / minBin, p0));
                int end = (int) Math.round(minBin * Math.pow((double) maxBin / minBin, p1));
                if (end <= start) end = start + 1;
                if (end > maxBin) end = maxBin;
                bandStartIndices[i] = start;
                bandEndIndices[i] = end;
            }
        }
        ...
        public synchronized List<Double> process(float[] samples) {
            if (samples == null || samples.length < n) {
                return null;
            }
            for (int i = 0; i < n; i++) {
                int rev = bitReversal[i];
                real[rev] = samples[i] * windowTable[i];
                imag[rev] = 0.0f;
            }
            return computeFftAndBands();
        }

        public synchronized List<Double> process(ByteBuffer buffer, int channelCount) {
            int frameBytes = Math.max(1, channelCount) * 2;
            int availableFrames = buffer.remaining() / frameBytes;
            if (availableFrames <= 0) {
                return null;
            }

            int framesToRead = Math.min(n, availableFrames);
            for (int i = 0; i < framesToRead; i++) {
                short left = buffer.getShort();
                short right = channelCount > 1 ? buffer.getShort() : left;
                for (int c = 2; c < channelCount; c++) {
                    buffer.getShort();
                }
                float mono = (((float) left + (float) right) / 2.0f) / 32768.0f;
                int rev = bitReversal[i];
                real[rev] = mono * windowTable[i];
                imag[rev] = 0.0f;
            }
            for (int i = framesToRead; i < n; i++) {
                int rev = bitReversal[i];
                real[rev] = 0.0f;
                imag[rev] = 0.0f;
            }
        ...
        private List<Double> computeFftAndBands() {
            for (int len = 2; len <= n; len <<= 1) {
                ...
                for (int i = 0; i < n; i += len) {
                    for (int j = 0; j < halfLen; j++) {
                        ...
                        float s = sinTable[k];
                        int idx1 = i + j;
                        int idx2 = idx1 + halfLen;

                        float uReal = real[idx1];
                        float uImag = imag[idx1];
                        float vReal = real[idx2] * c - imag[idx2] * s;
                        float vImag = real[idx2] * s + imag[idx2] * c;
            ...
            ArrayList<Double> result = new ArrayList<>(numBands);
            // Normalization divisor for 512-point FFT with Hann window:
            final double fftScale = n / 4.0;

            for (int b = 0; b < numBands; b++) {
                int start = bandStartIndices[b];
                int end = bandEndIndices[b];
                double sumMag = 0.0;
                for (int k = start; k < end; k++) {
                    float r = real[k];
                    float im = imag[k];
                    sumMag += Math.sqrt(r * r + im * im);
                }
                double avgMag = (sumMag / (end - start)) / fftScale;
                // Calibrated sensitivity: -38 dBFS floor for crisp silence on quiet parts
                double db = 20.0 * Math.log10(Math.max(1e-5, avgMag));
                double norm = (db + 38.0) / 36.0;
                if (norm < 0.0) norm = 0.0;
                if (norm > 1.0) norm = 1.0;

                // Visual contrast gamma (1.5): eliminates ambient fuzz, makes hits crisp & distinct
                norm = Math.pow(norm, 1.5);

                // +3.0 dB / octave spectral tilt (pink noise compensation)
                double tilt = 1.0 + 0.55 * Math.pow(b / (double) (numBands - 1), 0.70);
                double normalized = Math.min(1.0, Math.max(0.0, norm * tilt));
